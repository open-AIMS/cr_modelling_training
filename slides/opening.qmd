...
contain screenshots of publisher-typeset journal pages, which is the question
`notes/image-provenance.md` is open on, and photographs of AIMS staff.
...
::: {.notes}
Second: mixture distributions for bi-modal sensitivity data.
:::
...
::: {.column width="52%"}
`bayesnec` was developed at AIMS by myself and Diego Barneche, and is described
in *Journal of Statistical Software* 110(5).

It fits concentration-response models in a Bayesian framework through `brms` and
Stan, and returns NEC, NSEC and ECx estimates averaged over a candidate set of
equations.
...
nothing is sampled from scratch in the room. Every fit appears as three steps:
the `bnec()` call, the `save()` that keeps the result, and the `load()` that
reads it back. The call and the save are shown; the load is what runs.
