...
#> fit-beta -- not run here: fitting this takes about 76 s.
#> Remove the leading # to fit it yourself; the load() below restores the saved object.
# set.seed(333)
...
#> fit-beta-disp -- fits the model here, in about 45 s, most of it compiling.
set.seed(333)
exp_2_disp <- bnec(yield ~ crf(log.x, model = "ecxwb1") + disp("power"),
                   data = pam_data, seed = 333)

# save(exp_2_disp, file = "vignettes/fits/m5_exp_2_disp.RData")   # not run: it would overwrite the distributed fit

#> load-fit-beta-disp
#> This restores the distributed object, so what follows matches the page.
load("vignettes/fits/m5_exp_2_disp.RData")

#> fig-checkfit-beta-disp
...
#> fit-pois -- fits the model here, in about 36 s, most of it compiling.
set.seed(333)
exp_3 <- bnec(density ~ crf(log.x, model = "ecxwb1"),
              data = alga_data, family = "poisson", seed = 333)

# save(exp_3, file = "vignettes/fits/m5_exp_3.RData")   # not run: it would overwrite the distributed fit
...
#> fit-negbin -- fits the model here, in about 38 s, most of it compiling.
set.seed(333)
exp_3b <- bnec(density ~ crf(log.x, model = "ecxwb1"),
               data = alga_data, family = "negbinomial", seed = 333)

# save(exp_3b, file = "vignettes/fits/m5_exp_3b.RData")   # not run: it would overwrite the distributed fit
...
#> fit-gamma -- fits the model here, in about 36 s, most of it compiling.
set.seed(333)
exp_4 <- bnec(rlu ~ crf(log.x, model = "ecxll3"),
              data = lum_data, family = "Gamma", seed = 333)

# save(exp_4, file = "vignettes/fits/m5_exp_4.RData")   # not run: it would overwrite the distributed fit
...
#> fit-gauss -- not run here: fitting this takes about 53 s.
#> Remove the leading # to fit it yourself; the load() below restores the saved object.
# set.seed(333)
...
#> fit-disp -- fits the model here, in about 37 s, most of it compiling.
set.seed(333)
