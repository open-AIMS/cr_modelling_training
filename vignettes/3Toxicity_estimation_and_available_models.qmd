...
those beginning `ecx` are smooth models with no threshold. Fitted to the same
response range the two shapes cross twice, so neither is uniformly the more
conservative.
...
lower value. *Evidence from the simulation study* below measures how large that
difference becomes.
...
## Model averaging and the N(S)EC

Where the data show a threshold, the *NEC* is the most defensible estimate of no
effect. Threshold models do not always fit well, however, and fitting one to
...
Identifying which shape the data have is itself hard. @Krull2020 simulated
...
evidence in favour of the generating model.

Model averaging over a set containing both kinds of equation removes the need to
make the choice in advance. A `nec` equation in the set contributes its *NEC*
parameter; an `ecx` equation contributes its *NSEC*; and the averaged estimate
weights each by how well that equation fits the data. The averaged quantity is
written **N(S)EC**, to record that it mixes the two [@fisheretal2023].
...
Module 4 covers how the N(S)EC is produced. It fits a set with `bnec()`, sets
out how the weights are calculated, and shows the summary in which `bayesnec`
warns that an averaged no-effect estimate contains *NSEC* values.
...
individuals per replicate. Three findings from it change how a study is laid out
and what is reported from it.
...
Because the generating curve was known, the true effect size corresponding to
each estimate could be measured.

Where a threshold equation was fitted to smoothly declining data, the *NEC* it
returned represented an actual effect of 15% to 40% depending on the design, and
that percentage did not necessarily fall as sampling effort increased, because
the problem is the shape of the equation rather than the amount of data.
