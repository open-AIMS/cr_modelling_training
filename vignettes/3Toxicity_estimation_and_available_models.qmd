...
about either choice. `bayesnec` fits a set of equations, and each supports a
different set of toxicity estimates. The main division is between models with a
...
![The threshold equations. Each is flat, or rising, up to a breakpoint and then declines, and the breakpoint is the *NEC*. They differ in the decline after it and in whether the response rises first.](images/vignette-fig-exmp2b-theoretical_nec_curves.jpg){#fig-necmods width=75% fig-align="center"}

![The smooth equations. None has a breakpoint, so the response changes from the lowest concentration onwards and each supports an **EC~x~** or an *NSEC* rather than an *NEC*. The two `ecxhormebc` equations rise before they decline.](images/vignette-fig-exmp2b-theoretical_ecx_curves.jpg){#fig-ecxmods width=75% fig-align="center"}

Fitted to the same response range the two shapes cross twice, so neither is
uniformly the more conservative.

![A threshold curve and a smooth curve fitted to the same response range. The threshold curve is flat until the break and then falls steeply; the smooth curve is already declining at the lowest concentrations. Reproduced from @fisherfox2023 under CC BY-NC 4.0.](images/etnc_fig1.jpg){#fig-etnc1 width=100% fig-align="center"}
...
| $\alpha$ | `slope` | The linear decay rate in `neclin` and `ecxlin`, or the rate of the initial increase in the hormesis models. |
...
![Panel A of @fig-etnc2. The *NEC* ($\eta$) is the breakpoint itself. The EC10 is read from the curve and sits above it.](images/necmod_fox2010.jpg){#fig-necmod width=90% fig-align="center"}
...
on which shape fits the data. Where the response really does decline smoothly, a
...
![Panel B of @fig-etnc2. There is no breakpoint, so the only estimate the curve supports directly is an **EC~x~**.](images/ecxmod.jpg){#fig-ecxmod width=90% fig-align="center"}
...
![Panel D of @fig-etnc2. The *NSEC* is where the fitted curve crosses a lower quantile of the control posterior. The EC10 for the same curve sits above it.](images/nsec.jpg){#fig-nsec width=90% fig-align="center"}
...
is a no-effect estimate in any practical sense, or an estimate of a low
effect.
...
# Pass the median alone: given all three quantiles, ecnsec() returns a
# different and wrong answer (6.7% rather than 1.3% on this fit).
ecnsec(bnec_fit, nsec = nsec_value["Q50"])
```
...
The *NSEC* is analogous to the *NOEC*: both are defined by a comparison with the
control, and each is the highest concentration at which the response is not
significantly different from it.

![Panel C of @fig-etnc2. The *NOEC* is one of the tested concentrations, chosen by a test on the treatment means with no curve fitted.](images/noec.jpg){#fig-noec width=90% fig-align="center"}
...
and on the fitted model reproducing the variability the data show, which
`check_fit()` measures. Module 5 covers both.

Control variability limits an **EC~x~** in the same way. @mebane2015, summarising
problem cases identified by Green and colleagues, states the limit directly:
...
method in an Australian guideline derivation. And the effect-size condition
described with the ECNSEC above applies to it, as it does to a *NOEC*.

@warne2008 argued that *NOEC* data should no longer be generated or used. The
2025 method lists the *NOEC* as a less preferred estimate, below the *NEC*, the
*NSEC* and an **EC~x~** at *x* of 10 or less [@warne2025].
...
estimate, because it reports the concentration producing an effect of magnitude
...
These assume an increase up to the threshold, linear in `nechorme`, `nechorme4`
and `neclinhorme` and curved in `nechormepwr`, `nechorme4pwr` and
`nechormepwr01`, and allow various decay functions after it. Hormesis raises no difficulty for the *NEC*, which remains
the concentration at which the response begins to decline.

![Two threshold hormesis equations. The response rises linearly to the threshold and declines after it, so the *NEC* is still the breakpoint.](images/horme_nec.JPG){#fig-hormenec width=85% fig-align="center"}
...
![Two smooth hormesis equations. The curve peaks above the control response, so an effect measured from the control and one measured from the peak differ.](images/horme_ecx.JPG){#fig-hormeecx width=85% fig-align="center"}
...
changes the reported **EC~x~** for a hormesis equation and no other. Panel D
of @fig-ecx-types draws the two references on a hormetic curve.

## Extracting estimates

Three functions extract toxicity estimates from a fitted model: `nec()`, `ecx()`
and `nsec()`. The arguments described below are those that require a decision;
the help files, which are the authoritative source, document the rest.
...
The distance from the control to the reference is the **span** the percentage is
taken over, and the four types differ only in it.

```{r}
#| label: fig-ecx-types
#| echo: false
#| fig-height: 5.8
#| fig-cap: "The span each `type` measures over, and the EC10 each one gives. The dashed or dotted horizontal line is the lower end of the span, the arrow is its length, the solid horizontal line is the 10% target and the point is where the curve reaches it. (A) `ecxll4` with `bot` at 0.3, tested to a concentration of 10, so the curve reaches `bot` within the range. (B) the same equation at the same parameters, tested only to 3. (C) `ecxll3`, which has no `bot`. (D) `ecxhormebc4`, whose maximum of 1.32 is above its control of 0.99. The curves are drawn from the equations at fixed parameters, with no model fitted."
...
In panel A of @fig-ecx-types the `ecxll4` curve reaches its `bot` of 0.3 within
the concentration range, so the lowest response predicted and the theoretical
asymptote are the same value. `"relative"` and `"range"` both return an EC10 of
1.61, and `"absolute"`, whose span is 43% longer, returns 1.86.
Panel B is the same equation at the same parameters, tested only to a
concentration of 3. The curve is still at 0.65 there, so the `"range"` span is
less than half the length of the `"relative"` span and the EC10 falls to 1.16.
`"relative"` and `"absolute"` are unchanged at 1.61 and 1.86. A `"range"`
estimate is a property of the design as well as of the fit, and two laboratories
testing to different maximum concentrations will report different values from
the same equation.

Panel C is `ecxll3`, which has no `bot` and declines towards 0. All three
references fall in the same place and the three types agree at 1.61. Panel D is
`ecxhormebc4`. Measured from the control at 0.99 the 10% target of 0.89
lies below the rising limb, and the curve reaches it once, at 3.10. Measured
from the maximum of the curve at 1.32 the target of 1.19 lies above the control,
and the curve crosses it twice, at 0.77 and 2.50. A target crossed twice does
not identify a single concentration, which is the reason for measuring from the
control.

Three changes arrived in `bayesnec` 2.2.0 and matter when returning to an older
analysis.

`"range"` computes what `"relative"` computed up to version 2.1.3, with one
difference: the old span was taken from the maximum of the predicted curve
rather than from the control. The two agree wherever the maximum is at the
lowest concentration, and differ for a hormetic curve, which is panel D of
@fig-ecx-types. Passing `type = "relative"` explicitly now raises a warning
naming `"range"`.

`"relative"` is refused where the equation has no `bot` and the family is
unbounded below. Panel C of @fig-ecx-types assumes a family bounded below, so
the 0 that `ecxll3` declines towards is a finite reference and the estimate
agrees with `"absolute"`. Under a gaussian response, the only family `bnec()`
accepts that is unbounded below, there is nothing finite to measure towards, and the call is refused with a message naming
`"absolute"` and `"range"` as the two types that still apply.

The `hormesis_def` argument has been removed from `ecx()`, `nsec()`, `ecnsec()`
and the comparison functions. It selected between the control and the peak as
the reference, and with the control always the reference it selected nothing. A
call still passing it is refused by name rather than ignored.
...
whether the statistical model fits the data, rather than to adjust the
...
@fig-etnc2 sets the four estimates against one another.

![The four toxicity estimates. (A) A threshold model, giving the *NEC* directly as a parameter and an EC10 by interpolation. (B) A smooth model, giving an EC10 only. (C) The *NOEC*, read from the treatment means. (D) The *NSEC*, read from a smooth model against a lower quantile of the control posterior. Reproduced from @fisherfox2023 under CC BY-NC 4.0.](images/etnc_fig2.jpg){#fig-etnc2 width=100% fig-align="center"}
...
EC10 for the same curve above it. In both panels A and D the EC10 is the larger
...
![Toxicity estimates used for no and low effects in species sensitivity distribution modelling. Reproduced from @fisheretal2023 under CC BY 4.0.](images/Fisher_IEAM_Table1.png){#tbl-definitions width=100% fig-align="center"}
