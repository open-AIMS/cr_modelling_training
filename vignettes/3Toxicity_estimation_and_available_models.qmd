...
![The threshold equations. Each is flat to a breakpoint and then declines, and the breakpoint is the *NEC*. They differ in the decline after it and in whether the response rises first.](images/vignette-fig-exmp2b-theoretical_nec_curves.jpg){#fig-necmods width=75% fig-align="center"}

![The smooth equations. None has a breakpoint, so each declines from the intercept onwards and supports an **EC~x~** or an *NSEC* rather than an *NEC*.](images/vignette-fig-exmp2b-theoretical_ecx_curves.jpg){#fig-ecxmods width=75% fig-align="center"}
...
data show, which `check_fit()` measures and module 5 covers.
...
unbounded below. In panel C of @fig-ecx-types the family is bounded below, so
the 0 that `ecxll3` declines towards is a finite reference and the estimate
agrees with `"absolute"`. Under a gaussian or student response there is no lower bound,
nothing finite to measure towards, and the call is refused with a message naming
