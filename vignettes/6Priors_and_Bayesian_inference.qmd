...
that can look like an unnecessary complication, since frequentist solutions exist
for most of it, `drc` among them. The extra overhead has probably held back the use
...
likelihood fit does not make. Stating them, checking them and measuring how much
they matter are covered from *Priors* to *Fixing a parameter*, and *Using
posterior distributions* returns to the draws.
...
Many packages implement MCMC, among them `WinBUGS` [@Lunn2000], `JAGS`
[@Plummer2003] and `Stan` [@Carpenter2017], reached from R through `R2WinBUGS`,
`R2jags` [@Su2015] and `rstan` [@rstan2021].
...
`Stan` uses Hamiltonian Monte Carlo with the no-U-turn sampler, which sets the
number of steps in each trajectory automatically [@hoffman2014]. The step size is
tuned during warm-up towards a target acceptance rate, which `brms` exposes as
...
Where real prior knowledge exists, such as pilot data or a previous experiment on
the same response, the prior may reasonably be narrow, and will then be influential,
...
Two sets are supplied, `"uninformative"` and `"regularizing"`, and `prior_type`
selects between them. Its default is:
...
The `bayesnec` NEWS file records 420 `top` and `bot` entries built from simulated
responses across ten seeds, three designs and five families, on designs with
complete effect at the highest concentration or a hurdle or zero-inflated
response. The true value fell outside the central 95% of its own prior in 12
entries under the narrower set and in 127 under `"uninformative"`. Expect it to
become the default in a later version, and state which set was used either way. One assumption is worth checking before
...
responds to what is entered for the control: on the `nassarius` contaminant A
series the priors article reports a standard deviation of 2.30 with the control
recorded as `0` and 6.11 with it recorded as `1e-6`. A control recorded as `0` is
left out of the construction, while `1e-6` states that a concentration of
`1e-6` was applied. Record a control as `0`.
...
They can fail in three ways. They may be too informative, giving unreasonably tight
...
the `prior` argument, as *Fixing a parameter* does. For a model already fitted,
`get_priors()` returns the priors it used in the same form. `pull_prior()`
returns the whole `brmsprior` stored in the fit, `brms` defaults included, which is
for inspection and is not accepted by `bnec(prior = )`.
...
the prior against the posterior for each parameter.
...
the one to read carefully, because its spike sits at about 0.5, close to the
centre of its `normal(0, 5)` prior on the scale of the panel. Agreement between
a posterior and the centre of its prior is not evidence that the prior determined it; what settles
...
`nec4param` is used rather than `nec3param` so that the comparison includes
`bot`, which `nec3param` does not have. On `nec_data` the `"regularizing"` set
narrows and relocates the priors on `top` and `bot` and narrows the one on
`nec`, and leaves the prior on `beta` unchanged. Compare the result with the same
equation fitted under the default set. Posteriors that agree between the two
...
The posteriors are similar, with medians of 1.535 under the default priors and
1.533 under the user priors, which is what should happen when the data are
...
`avoid_data` stands for a two-choice avoidance dataset of your own; `bayesnec`
does not ship one.
...
A comparison between two estimates is a count of draws rather than a test
statistic, because both estimates arrive as whole distributions. `compare_posterior()` does this
...
equation with three free parameters (`top`, `beta` and `ec50`), as `nec3param`
has three (`top`, `beta` and `nec`), and the same lower asymptote fixed at
zero, so the two fits differ in the shape of the curve rather than in what each is free to estimate.
...
its threshold a little above 1.5 and turns sharply there; `ecxll3` has begun to
...
0.089 at a concentration of 1.85, on observed responses spanning 0.001 to 0.98. That is
