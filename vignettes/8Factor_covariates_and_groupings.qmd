...
Weight on the interaction says the levels differ by more than the shared curve
can absorb. Weight on the simpler fit says they do not, and that one curve
...
That figure shows the limitation of this route, and it is the reason the two
approaches are both presented. Fitting an interaction this way assumes the
functional form is the same at every level, and only the parameter values differ.
Here the highest hardness treatment is not described well by the shape the pooled
data selected.

Where that happens, fit the levels separately with `bnec_group()`, which lets the
shape differ, and compare the results as below. The alternative, generalised
