...
ogl_curves <- read.csv("data/grouping_ogl_curves.csv") |>
...
Where a grouping spans the whole concentration range, the shape of the curve can
differ between levels and not only its height. `pgl()` places a deviation on
every parameter of the equation at once, which is what allows that.
...
analysis treating them as independent reports effect doses biased downwards on
intervals that are too narrow. In their simulation, fitting the normalised response as though the observations
were independent gave ED10 bias of 2.6 to 6.8 per cent, on intervals covering
the true value 0.88 to 0.91 of the time against a nominal 0.95. Modelling the
raw rates gave 0.7 to 2.1 per cent and 0.94 to 0.96. They report that weights do not remedy it, and
that per-curve normalisation resting on few control measurements is the worst
case. This assay normalises each plate by four control wells.
...
All of that assumes gain is the whole of the difference between plates. *The
plate term compared* below fits the three terms and tests that.
...
these data do not. Taking the four wells at each concentration on each plate,
...
One equation is fitted three ways, so that what is compared is the group-level
...
read.csv("data/grouping_plate_compare.csv") |> kable()
```

`pgl(plate)` takes the whole weight. Its expected log predictive density
exceeds `ogl(plate)`'s by 297.9 with a standard error of 27.0, and
`ogl(plate)`'s exceeds the ungrouped fit's by 266.5. Both differences are about
ten times their standard error, which is a different situation from the coral
colour comparison above, where the same test could not separate two fits.

Two of the 704 observations have a Pareto k above 0.7 under `pgl(plate)`, and
none do under the other two. The estimate for those two points rests on the
importance-sampling approximation rather than on a refit, and two points of 704
will not reverse a difference of this size.

The result is worth pausing on, because a per-plate gain would not produce it.
Gain multiplies the whole reading, and a fit whose plates differ only by gain
would be described by `ogl(plate)` and gain nothing from a deviation on every
parameter. The plates differ in more than their height.

```{r}
#| label: fig-plate-curves
#| fig-height: 4
#| fig-cap: "The same equation with no plate term, with ogl(plate) and with pgl(plate). Points are the observed readings, the line the fitted curve, the band its 95% credible interval and the dashed line the N(S)EC."
plate_levels <- c("no group term", "ogl(plate)", "pgl(plate)")
plate_curves <- read.csv("data/grouping_plate_curves.csv") |>
  mutate(fit = factor(fit, levels = plate_levels))
plate_obs <- read.csv("data/grouping_plate_obs.csv")
plate_nsec <- read.csv("data/grouping_plate_estimates.csv") |>
...
The three panels differ in the width of the credible band and in where the
N(S)EC rule falls. Ungrouped, the band around the control is narrow, because the
spread between plates has nowhere to go but the residual and the curve is
reported as well determined. With either plate term that spread is attributed to
the plates instead, and the control band widens towards the range the control
readings themselves span.
...
```{r}
#| label: tbl-plate-sd
#| tbl-cap: "Group-level standard deviations. ogl() estimates one; pgl() estimates one for every parameter of the equation."
read.csv("data/grouping_plate_sd.csv") |> kable()
```

`ogl(plate)` puts the between-plate difference at 0.424 on the log scale the
deviation is applied on, which is a factor of 1.53 in luminescence. Under
`pgl(plate)` the largest deviations are on the two asymptotes, `botgl` at 0.648
and `topgl` at 0.465, which is the vertical displacement `ogl()` was describing.
The one that `ogl()` cannot represent is `ec50` at 0.356 on the log
concentration scale, a factor of 1.43 in the concentration at which the curve
reaches its midpoint. The plates differ in where the response falls as well as
in how much light they read, and only `pgl()` fits that. `beta`, the slope, is
the most nearly common at 0.116.

Gain and genuine between-plate variation are confounded in all of these, as
*Normalising to a control* said. What the `ec50` deviation adds is that the
confounded quantity is not a single multiplicative constant: a difference in
potency between plates is not something a gain setting can produce.

```{r}
#| label: tbl-plate-estimates
#| tbl-cap: "Toxicity estimates from each fit, back-transformed to mg/L."
read.csv("data/grouping_plate_estimates.csv") |> kable()
```

The N(S)EC doubles across the three fits, from 0.041 mg/L ungrouped to 0.073
with `ogl(plate)` and 0.084 with `pgl(plate)`, and its lower bound rises from
0.009 to 0.054. EC10 changes much less, from 0.056 to 0.054 to 0.063. A threshold
is the estimate the plate structure decides, because it is read where the curve
leaves the control and that is where the between-plate spread is largest; an
EC10 is read further down a curve the plates agree about more closely.

An analysis that treated these 704 readings as independent would report a
no-effect concentration half the one the plate structure supports, on a lower
bound a factor of six too low.
...
separately answers that question instead of assuming it away.
...
better description of the copper plates. This call is the one
`vignette("example8")` in `bayesnec` makes, and it is reproduced here so that
the estimates below can be read against the ones published with it. A `pgl()`
version would estimate four group-level standard deviations at each of two
levels over fifteen equations, and nothing in this module has been fitted to say
what it would change.

This is the one call in the module that names its sampler settings, and it names
them because it is the call `vignette("example8")` in `bayesnec` makes. That
vignette's fits are run on a cluster and stored, and this module reads the
stored fit rather than sampling for the hours the call takes here. The closing
section says how. Every other fit in this module samples at `bnec()`'s
defaults.

```{r}
#| label: tbl-tox-weights
#| tbl-cap: "Model weights at each level. Equations below 0.01 at both levels are omitted."
read.csv("data/grouping_tox_weights.csv") |>
...
tox_nsec <- read.csv("data/grouping_tox_estimates.csv") |>
  filter(estimate == "NSEC") |>
  rename(fit = toxicant)

tox_curves <- read.csv("data/grouping_tox_curves.csv")
...
zinc, on responses that are not even on the same scale. What differs is where
...
read.csv("data/grouping_tox_posterior.csv") |>
...
read.csv("data/grouping_tox_prob_diff.csv") |> kable()
```
...
end, and zinc's from 0.227 to 1.243, a factor of 5.5. Copper is both the more
...
`bnec_group()` does what the earlier version of this module did with a
hand-written loop over the levels of the factor, and adds three things a loop
does not.
...
Each level's fit gains a `failed_models` element naming the equations that did
not fit there and the message each returned, so a level that lost part of its
set is visible rather than silently averaged over fewer equations. Both levels
here lost `ecxhormebc5`, which is the initial-value problem of `bayesnec` #344
on a logged predictor that reaches negative values, so each average is over 14
of the 15 equations rather than all of them.
...
The price is run time. Fifteen equations at each of two levels is thirty fits,
and this is the point at which fitting stops being something done at render
time.
...
0.902 out of six candidates, so one equation holds almost all the weight and
the route below is a route for that equation rather than for the set.
...
#| echo: false
#| comment: ""
cat(readLines("data/grouping_zinc_inter_sampling.txt"), sep = "\n")
```
...
read.csv("data/grouping_zinc_group_weights.csv") |>
...
and 0.434 to `ecxwb1`. Hardness 5 spreads its weight over nine equations, the
largest being `ecxll3` at 0.403, and gives `ecxwb1` 0.146.
...
parameters differ, so it has no way to report a disagreement of this kind. The
fit above reports it without being asked.

```{r}
#| label: tbl-zinc-group-estimates
#| tbl-cap: "Toxicity estimates at each hardness, back-transformed to zinc concentration."
read.csv("data/grouping_zinc_group_estimates.csv") |> kable()
```
...
zinc_group_curves <- read.csv("data/grouping_zinc_group_curves.csv") |>
...
comparisons disagree about it. Their threshold posteriors overlap, at a pairwise
...
This is what a factor covariate is asked for, as a probability computed from the
draws rather than as a significance test, and it does not require the levels to
share a curve shape.

## Provenance of these fits

None of the fitting above runs when this page is built. The two `bnec_group()`
calls are dozens of fits between them, the copper plate set is eleven equations
of the better part of an hour each, and the interaction models are slow on top
of that. A participant waiting for a chunk has stopped learning.
...
is the call `vignette("example8")` in `bayesnec` makes, and that vignette's fits
are run as cluster array tasks by the `open-AIMS/grouping-structures` compendium
and kept in a store keyed on the call and a digest of the data. The script reads
...
Every other fit samples at `bnec()`'s defaults: 4 chains at 10000 iterations
...
`lum31` reaches `bayesnec` through pull request #228 and is not on the `dev`
branch until that merges. Until then this module needs that branch installed
rather than the version module 1 names.
