...
[e8]: https://open-aims.github.io/bayesnec/articles/example8.html
...
It is paid for twice. The first price is the one *The number of levels a
deviation needs* gives: four standard deviations estimated from sixteen levels
here, where `ogl()` estimates one. The second is run time, because those four
...
equations at each of two levels, which is thirty fits. The `open-AIMS`
compendium that runs this vignette's fits measured 22.2 hours of sampling across
its 189 units under `ogl(plate)`, and a `pgl()` version of this call would add
those parameters to thirty of them.
...
them for the same reason. That vignette's fits are run as cluster jobs and
stored, and this module reads the stored fit rather than sampling. The closing
...
rather than the version module 1 names, and the two links above to the grouping
and factor covariates vignette resolve only once that vignette is published with
it.
