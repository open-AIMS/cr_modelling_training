...
#| fig-align: center
#| echo: false
#| fig-cap: "The N(S)EC. *NEC* values from threshold equations and *NSEC* values from smooth equations are combined by weight into one estimate, shown on the right against the *NEC* and *NSEC* it sits between. Reproduced from @fisheretal2023 under CC BY 4.0."
...
`update()` weight a set as `bnec()` does, and the method used is recorded on the
weights as `attr(mod_stats$wi, "method")`. Checked under `bayesnec` 2.1.3.39
with `loo` 2.8.0: the two fits in the packaged `manec_example`, pulled out and
recombined with `c()`, report `pseudobma`, as does the set fitted in one
`bnec()` call. Weights from an older analysis are not necessarily comparable
with weights computed now, which is one more reason to record the package
version alongside the models fitted.
...
`failed` is `TRUE` where any of the three diagnostics misses its cutoff. All
three cutoffs are arguments to `check_sampling()`. Module 7 sets out what each of
the three measures, why the number of draws retained changes what two of them
demand, and what to do about a model that misses one.
...
An equation can also fail earlier, by producing no draws at all, and
`failed_models()` names those. `bnec_record()`, from module 2, completes the
account by naming any equation excluded before fitting and the reason for it. A
set requested as `"decline"` or `"all"` commonly excludes several.

What to do about a model the screen removed depends on which cutoff it missed,
and an equation that produced no draws is a different case again. Module 7 works
through both against two datasets that ship with the package, which between them
produce every outcome. It sits there rather than here because reading a divergent
...
#| fig-align: center
#| echo: false
#| fig-cap: "@fisheretal2023, the source of the simulation and case study results in this section. The paper is open access under CC BY 4.0, and the code for every result in it is at <https://github.com/open-AIMS/NSEC_IEAM>."
