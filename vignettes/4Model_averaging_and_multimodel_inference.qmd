...
`bnecfit`, which has two methods of its own, `+` and `c`. Either will append
fits to one another, so two or more `bayesnecfit` objects can be combined into a
model-averaged `bayesmanecfit`.
