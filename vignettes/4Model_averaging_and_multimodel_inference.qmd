...
control poorly. That is `check_fit()`, named in module 2 and set out below,
applied here to every equation in the set and reported where it fails. It is a
statement
...
worth putting through the fit checks below, since it contributes most to the
reported estimate.
...
Reading plots for a set of twenty-three is not a workable check. Module 2 named
`check_sampling()` on a single fit; on a set it returns one row per model:
...
`failed` is `TRUE` where any of the three diagnostics misses its cutoff.
...
to 100 draws per chain at the four chains `bnec()` fits. Read `min_ess_ratio`
beside it: the ratio separates a model that cleared 400 because a great many
draws were taken from one that cleared it efficiently.
...
Everything above asks whether the sampler explored the posterior. None of it
asks whether the fitted model reproduces the data, and those are different
questions: a model can converge perfectly onto a description of the data that is
wrong. `check_fit()`, named in module 2, answers the second, and on a set it
returns one block per equation.
...
Each row compares a statistic computed from the observations in a group of the
predictor against the same statistic computed from data simulated by the fitted
model.

- `mean_ratio` and `sd_ratio` are the observed values divided by the simulated
  ones.
- `ppp_mean` and `ppp_sd` are posterior predictive p-values, each the proportion
  of simulated datasets in which the simulated statistic was at least as large
  as the observed one. A value near 0.5 says the observed statistic sits in the
  middle of what the model produces, and a value near 0 or 1 says it sits in a
  tail.
- `control` flags the lowest value of the predictor, which is the group both
  `nsec()` and `ecx()` read their reference from. That row is the one to look at
  first.
- `wi` is the stacking weight of the equation the row belongs to, so a failure
  can be read against how much the averaged estimate depends on it.

Read the p-values rather than the ratios, because a ratio says nothing about how
well determined it is.

The control rows of the three equations state the case for checking each
equation separately rather than the average:

```{r}
#| label: check-fit-control
subset(as.data.frame(cf), control,
       select = c(model, wi, group, mean_ratio, ppp_mean, sd_ratio, ppp_sd))
```

`nec3param` and `ecxll3` reproduce the control almost exactly, with `sd_ratio`
at 0.99 and 0.95. `ecxexp` simulates nearly three times the spread the data
show, at `sd_ratio` 0.37 and `ppp_sd` 0.97, and it does so across every group
rather than only at the control. This is the warning the summary raised earlier
in the module, now located. `ecxexp` holds a stacking weight of 4e-39, so it
reaches the averaged estimate not at all; the same result against `nec3param`,
which holds 0.81, would stop the analysis.

```{r}
#| label: fig-check-fit-set
#| fig-height: 6
#| fig-cap: "Observed mean and standard deviation in each group against the 95% span of what each equation simulates."
plot(cf)
```

Each panel shows the observed statistic against the 95% span of what the model
simulates, and a point outside its bar is a group the model does not reproduce.
Location and scale are drawn separately because they fail independently. Every
group of `ecxexp` falls outside its bar on scale, and its highest bin falls
outside on location as well. `ecxll3` and `nec3param` reproduce both statistics
in every group.

::: {.callout-note collapse="true" title="Why one number for the whole fit will not do"}
...
A single summary statistic for the whole fit will not detect this. Every family
with a free dispersion parameter estimates that parameter from the same
residuals such a number would summarise, so the parameter absorbs the
discrepancy: the fit reports close agreement overall while simulating the wrong
spread at the concentrations that determine the estimate. The check is therefore
made within groups of the predictor.
:::

::: {.callout-note collapse="true" title="Bins and design points"}

The predictor in `nec_data` is continuous, with a different value for every
observation, so there are no repeated concentrations to group by. `check_fit()`
cuts the range into ten bins and says so in a warning. A bin is not a design
point, and a single row of a binned table is indicative rather than conclusive.
On a real dilution series the groups are the tested concentrations and no
binning happens; pass `group` explicitly where the binning is not what you want.
:::

::: {.callout-note collapse="true" title="The message and the bars use different criteria"}

The message flags a posterior predictive p-value outside 0.05 to 0.95, and the
bars in the plot are the 95% span, which is 0.025 to 0.975. A group can fall
beyond the first and inside the second, so the two can disagree. Read the
message as a prompt to look at that group rather than as a verdict on the fit.
:::

Module 7 shows what both checks contribute to a written analysis, on real data.
