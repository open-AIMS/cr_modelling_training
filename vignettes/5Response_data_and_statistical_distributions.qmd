...
and the family choice set out in the rest of this module avoids it.
...
```{r}
#| label: fig-ritz
#| fig-cap: "Ritz, Gerhard and Streibig (2026), the source of the measurements in this section."
#| out-width: "70%"
#| fig-align: center
#| echo: false
knitr::include_graphics("images/Ritz_etal2026.png")
```
...
explicitly where it changes. The sections below cover both halves, the choice of
family and the `disp` term for a dispersion that varies across the series.
...
`type = "dens_overlay_grouped"`. Ten posterior draws are
...
`check_fit()`, named in module 2 and set out in module 4, puts numbers on the
same comparison within each concentration group, and flags the control row. That
row is the one to read first, because both `nsec()` and `ecx()` take their
reference from the fitted mean at the control.
...
run over every equation in the set and reported where it fails, and
...
figures. This is the mechanism `check_fit()` measures, set out in module 4.
...
judgement, and module 4 gives the two functions that make the check.
