...
model that is right on average and wrong where it matters. Two functions make the
comparison within concentration groups instead.

`pp_check()` compares data simulated from the fitted model against the data
observed, and grouping it by concentration is what makes the comparison
informative: pooled over the whole dataset a model can match the overall
...
#| fig-height: 5
#| message: false
brms::pp_check(pull_brmsfit(exp_1nec), type = "bars_grouped", group = "log.x")
```

The dark line is the observed count in each category and the light intervals are
the same quantity simulated from the fit, one panel per concentration. The
grouping variable has to be one the model was fitted on, so the panels are
labelled with logged concentrations: -2.30 is the 0.1 control and 5.99 the
highest dose of 400. A discrete response takes `type = "bars_grouped"`; a
continuous one takes
`type = "dens_overlay_grouped"`. Ten posterior draws are simulated by default,
which is enough to see a gross mismatch and few enough to plot; raise it with
`ndraws` to judge a marginal one.

`check_fit()`, from module 2 and set out in module 4, puts numbers on the same
comparison and reports it per equation in the set:
...
The groups are again labelled on the logged scale. No group is flagged on the
mean for either equation. On spread, only the
