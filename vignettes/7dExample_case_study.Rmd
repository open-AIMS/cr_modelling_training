...
We will look at a `drc` comparison to the same herbicide toxicity data used for the `bayesnec` case study. The data we analyse in this example are from @jones2003meps, and they are included as an example dataset in `bayesnec`.
...
All model parameters are significantly different to 0.
...
Log-normal 4 parameter and log-logistic 3 parameter are the best fitting models. 

```{r, warning=F}
ametryn_LN4 <-  drm(fvfm~concentration, data=ametryn, fct=LN.4(names = c("Slope","Lower Limit","Upper Limit", "Midpoint")))

plot(ametryn_LN4, type='all')
plot(ametryn_LL3, add=TRUE,col="orange",lty=1, lwd=2)

```
Basically the same, just the tail is a bit different. 

## Comparing ECx, NEC, and NSEC

To get a NEC we need to plot a NEC model 

```{r, warning=F}
ametryn_nec4 <-  drm(fvfm~concentration, data=ametryn, fct=NEC.4())
plot(ametryn_nec4)

```
The model appears to fit the data well. Let's Check the NEC (the 'e' parameter). We can use `confint` function to calculate confidence intervals for the parameter. 

```{r}
summary(ametryn_nec4)
confint(ametryn_nec4, level = 0.95)
```
The **e** parameter is not significantly different from 0 and has a confidence interval range of -24 to 24. I.e., is nonsense.

Let's check what our model average calculates for an EC10. Remember that we cannot calculate an ECx value for a nec model so we have to exclude them from the average. 

```{r maed, echo=TRUE, warning=F}
maED(ametryn_LL3, 
    list(W1.3(),W2.3(), LN.3(), LN.4(),W1.4(),W2.4(), LL.4()),
    c(10), #multiple ECx values can be inputted here
    interval="kang")
```
Nearly all the weight is coming from the 4-parameter log-logistic model. 

## Extract NSEC

Dr Rebecca Fisher has developed a function to calculate the NSEC from drc models. 
...
ametryn_LL4 <-  drm(fvfm~concentration, data=ametryn, fct=LL.4())
plot(ametryn_LL4)
