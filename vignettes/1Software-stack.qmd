...
to use R for some things as a stand alone program, there are additional open-source 
third-party software that make R more intuitive and easy to use. Almost no one uses R by itself.

The course uses the Positron integrated development environment (IDE), 
which is a free and open-source software that provides a user-friendly interface to R.

It is available for Windows, MacOS, and Linux operating systems. It is the more 
recently preferred IDE for R, and is based on Visual Studio.
It will likely replace the popular RStudio IDE for most users in the future and it has some useful features that I think make it worth adopting now.

In Positron, a new session starts essentially empty. The console shows which R version (intepreter) has started. 

![Positron with a new R session. The console reports which R version available and started, and the
...
Probably the most important thing is the editor pane, because it is where you write your R code.
...
in the analysis, so that the results can be completely reproduced. Reproducibility is a serious problem in science, and
is particularly important in environmental sciences (or any science).
...
This may seem very opaque of you are not familiar with Bayesian statistics, 
...
it, but it is not what we are using here because cmdstanr is faster and more reliable.
...
Relatively complex models can be fit using `brms`, using a simple straightforward formula syntax. 
...
The below screenshot shows an example of what raw stan code looks like, to give you an idea of the kind of thing we are avoiding.
...
structure, censoring, and distributional regression — and provides the methods for
checking a fit once it exists. Why these are useful will be discussed in later modules.

**`bayesnec`** [@fisheretal2024] is the software package this course is focused  on. 
`brms` will fit a
...
appropriate to the response, fits the model. It then provides functions for extracting the toxicity estimates that we need for ecotoxicology from
the model fit in a straightforward way.
...
model of @Fox2010 using JAGS, which is another of the widely used Bayesian packages available in R,
based on one of the earliest Bayesian software packages, WinBUGS. 
...
Recording them is relatively straight forward:
...
ahead of the release on CRAN. There is a lot of functionality, as well as some significant other behaviour changes currently in the development branch that are worth using, particularly for modules 7 and 8.
...
development site carries an orange "In-development version" badge beside the
version number in its header, so which of the two is open can be read off the
page.

Check that the version in that badge matches what `packageVersion("bayesnec")`
reports in your session. The `dev` branch moves during the weeks a course is
being prepared, so a site ahead of an installed package documents behaviour the
